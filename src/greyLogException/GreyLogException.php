...
class GreyLogException extends \Exception {
     ...
     */
    protected $sApplication = 'SampleApplication';

    /**
     * The IP the GrayLog2-Server listens to.
     */
    protected $sLogServerIp = "192.168.1.39";
...
class Kernel {
    ...
    public function __construct($_stest = NULL) {
        if (1 === 1) {
            try {
                throw new KernelException(KernelException::TEST_ERROR, time());
            } catch (GreyLogException $le) {
                echo PHP_EOL."Success! Error was logged!".PHP_EOL;
            }
        }
    }
